...
public class MainController extends HttpServlet {
    ...
    private boolean isProjectAction(String action){
        return "addProject".equals(action)
                || "viewAllProject".equals(action)
                || "searchProject".equals(action)
                || "editProject".equals(action)
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if(isUserAction(getAction)){
                url = "/UserController";
            }else if (isProjectAction(getAction)){
                System.out.println(">>> Forward to ProjectController with action: " + getAction);
